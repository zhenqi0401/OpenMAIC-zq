          ...
          >
            ...
            <span
              className={cn(
                'min-w-0',
                !mobile &&
                  'md:absolute md:left-[60px] md:z-20 md:hidden md:w-44 md:rounded-[var(--admin-radius-control)] md:border md:border-[var(--admin-border)] md:bg-[var(--admin-surface)] md:px-3 md:py-2 md:shadow-[var(--admin-shadow-popover)] md:group-hover/nav-item:block md:group-focus-visible/nav-item:block xl:static xl:block xl:w-auto xl:border-0 xl:bg-transparent xl:p-0 xl:shadow-none',
              )}
            >
              <span className="block truncate">{module.label}</span>
              <span className={cn('mt-0.5 block text-xs font-normal', !mobile && 'xl:block')}>
                {module.description}
              </span>
            </span>
          </a>
...
export function AdminSidebar({ activeModuleId }: { activeModuleId: AdminModuleId }) {
  return (
    ...
    >
      ...
      <p className="mb-3 hidden text-xs font-semibold tracking-[0.08em] text-[var(--admin-muted-foreground)] xl:block">
        管理后台
      </p>
