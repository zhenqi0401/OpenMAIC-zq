 ...
 * `202 { jobId }`. The owner always comes from the verified admin session.
 */
export async function POST(req: NextRequest) {
  ...
  // owner from the verified session header below and ignores multipart identity, so
  ...
  try {
    ...
    const upstream = await proxyFetch(`${resolved.url}/render`, {
      ...
      headers: {
        ...
        'x-openmaic-client': current.identity.userId,
      },
