 ...
 *
 *   normalize → probe → timeline → geometry → assets → assemble
 *
 ...
 * consumer of it.
 *
 * Diagnostics from all passes are concatenated in pass order. The normalize
 * pass independently excludes every non-slide scene and unsupported action, so
 * quiz/interactive/PBL and whiteboard/discussion/widget content cannot enter the
 * VideoTimeline even if a caller bypasses browser preprocessing.
 *
 * Pure: no IO beyond the injected dependencies.
 */
import type { PlayVideoAction, Action } from '@/lib/types/action';
...
export function compileVideoTimeline(input: CompileInput, deps: CompileDeps): VideoTimeline {
  ...
  // 3. timeline — index→time expansion folded into per-scene buckets.
  ...
  const scenes: VideoTimelineScene[] = assets.scenes;
