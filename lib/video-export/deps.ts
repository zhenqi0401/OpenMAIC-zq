 ...
 * is structurally assignable to this. The normalize pass still double-checks
 * `type` and `content.type` before producing the IR.
 */
export type CompilerScene = {
  id: string;
  stageId: string;
  title: string;
  order: number;
  actions?: import('@/lib/types/action').Action[];
  type: SceneType;
...
export interface AssetSource {
  ...
  media(elementId: string, scene: CompilerScene): AssetMeta | null;
}
