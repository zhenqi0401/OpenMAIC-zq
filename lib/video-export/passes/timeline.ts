 ...
 * index→time expansion, blocking vs fire-and-forget, and effect-lifetime
 * clamping) and sorts the resulting flat segment list into each
...
export function buildTimeline(
  ...
  for (const seg of segments) {
    ...
    dispatchSegment(seg, bucket, opts, diagnostics, () => {
      ttsEnabled = true;
    });
  }
  ...
  return { scenes: irScenes, totalDurationMs, ttsEnabled, diagnostics };
}
    ...
    case 'speech': {
      ...
      if (!stored && speech.text.trim()) {
        diagnostics.push({
          ...
          actionId: speech.id,
          sceneId: seg.sceneId,
          message: 'Narration duration estimated because no usable audio duration was available.',
        });
      }
      return;
    }
