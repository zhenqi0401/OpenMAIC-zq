...
export const EMPTY_SLIDE_DWELL_MS = 2000;
...
export function estimateSpeechDurationMs(text: string, opts?: SpeechEstimateOptions): number {
  ...
  const rawMs = isCJK
    ? Math.max(EMPTY_SLIDE_DWELL_MS, text.length * CJK_MS_PER_CHAR)
    : Math.max(
        EMPTY_SLIDE_DWELL_MS,
        text.split(/\s+/).filter(Boolean).length * NON_CJK_MS_PER_WORD,
      );
  return rawMs / speed;
}
