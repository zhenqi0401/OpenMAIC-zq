 ...
 * check-then-increment is atomic within this single service process.
 */
...
export class RenderManager {
  ...
  async cancel(id: string, owner: string): Promise<boolean> {
    ...
    if (queuedIdx >= 0) {
      ...
      this.decrementIdentity(q.record.owner);
  ...
  private async run({ record, options, abort }: QueuedJob): Promise<void> {
    ...
    try {
      ...
      this.controllers.delete(id);
      this.decrementIdentity(record.owner);
      this.running--;
      this.pump();
    }
  }
