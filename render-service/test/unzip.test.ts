...
describe('unzipProject', () => {
  ...
  it.each(['..\\escape.txt', '/absolute.txt', 'C:\\windows.txt'])(
    'rejects unsafe portable path %s',
    async (unsafeName) => {
      const { unzipProject, InvalidProjectError } = await freshUnzip();
      const archive = zipSync({
        'index.html': strToU8('<!doctype html>'),
        [unsafeName]: strToU8('pwned'),
      });
      await expect(unzipProject(archive, dest)).rejects.toBeInstanceOf(InvalidProjectError);
    },
  );

  it('rejects unsafe directory entries instead of silently skipping them', async () => {
    const { unzipProject, InvalidProjectError } = await freshUnzip();
    const archive = zipSync({
      'index.html': strToU8('<!doctype html>'),
      '../escape/': strToU8(''),
  ...
  it('rejects Unix symbolic-link entries', async () => {
    ...
    const archive = zipSync({
      'index.html': strToU8('<!doctype html>'),
      link: [strToU8('target.txt'), { os: 3, attrs: 0o120777 << 16 }],
    });
    await expect(unzipProject(archive, dest)).rejects.toBeInstanceOf(InvalidProjectError);
  });
